...
if resp.get("ok"):
    for upd in resp["result"]:
        ...
        if text.startswith("/start"):
            ...
            if not site:
                send_message(chat_id, "❌ Only Amazon & Flipkart URLs")
                continue
            supabase.table("trackers").insert({
                "chat_id": str(chat_id), "url": url,
                "target_price": target, "site": site,
                "product_name": url.split("/")[-1][:30] or "Product"
            }).execute()
            ...
            send_message(chat_id, "📋 */add* - Track product\n*/list* - View trackers\n*/remove* - Delete\n*/help* - Menu")
